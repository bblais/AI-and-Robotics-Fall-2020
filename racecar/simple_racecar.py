# adapted from https://github.com/erwincoumans/pybullet_robots  f10_racecar.py
...
class CameraView(object):
    ...
    def update(self,t=None):
        ...
        if (nowTime-lastTime>.1):
            ...
            self.width,self.height,self.rgb,self.depth,self.mask=p.getCameraImage(320,200,viewMatrix=viewMat,projectionMatrix=projMat, renderer=p.ER_BULLET_HARDWARE_OPENGL)
            ...
            if self.use_plot:
                if self.fig is None:
                    ...
                    self.im=self.ax.imshow(self.rgb)
                    plt.show()
                else:
                    self.im.set_array(self.rgb)
                    plt.draw()
...
otherCar=Car([0,1,.3])
car=Car([0,0,.3])
view=CameraView(car,use_plot=False)
